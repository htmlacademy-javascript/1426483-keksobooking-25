...
const similarOffer = createOffers();
...
similarOffer.forEach(({author, offer}) => {
  ...
  cardOffer.querySelector('.popup__type').textContent =  CAPTIONS[offer.type];
  ...
  similarList.push(cardOffer);
});

// mapСanvas.appendChild(similarListFragment);
mapСanvas.append(similarList[6]);
